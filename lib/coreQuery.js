...
/**
 * 底层 query（不支持 'list[JSON].skills[JSON]'） 的多级JSON多结果查询
...
module.exports = function coreQuery(attrArray, data, settings) {
    /**/
    dc.group('%ccoreQuery', 'color:green;')
    dc.$input('coreQuery', attrArray, data, settings)
    /**/
    let defaultSettings = {
        complete: true,
        separator: '.',
    ...
    let valueArray = []
    /**/
    dc.group('attrArray.forEach')
    dc.log('attrArray', attrArray)
    /**/
    attrArray.forEach(function (key, index) {
        ...
        /**/
        dc.$json(`target = target[mediaData.attr] = `, target)
        /**/
    ...
    /**/
    dc.log(`emptyResult = ${emptyResult}`)
    dc.groupEnd()
    /**/
    /**
